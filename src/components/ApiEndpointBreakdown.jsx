...
const ApiEndpointBreakdown = ({ endpointsProp, isLoading, isError }) => {
	const data = endpointsProp || endpoints;
	...
	if (!data || data.length === 0) {
		return <EmptyState message="Nenhum endpoint encontrado." />;
	}
	return (
				...
				<tbody>
					{data.map((ep) => (
						<tr key={ep.path}>
							...
							<td
								style={{
									color: ep.errorRate > 1 ? "#dc2626" : "#64748b",
								}}
							>
							...
							<td
								style={{
									color: ep.successRate < 99 ? "#dc2626" : "#22c55e",
								}}
							>
							...
							<td>
								<span style={{ color: "#22c55e" }}>
									{ep.statusCodes["2xx"]}x2xx
								</span>{" "}
								<span style={{ color: "#f59e42" }}>
									{ep.statusCodes["4xx"]}x4xx
								</span>{" "}
								<span style={{ color: "#dc2626" }}>
									{ep.statusCodes["5xx"]}x5xx
								</span>
							</td>
						</tr>
					))}
